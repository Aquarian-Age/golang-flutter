...
class ClockPainter extends CustomPainter {
  ...
  ClockPainter(
    ...
    //init seconds offset
    for (var i = 0; i < 60; i++) {
      Offset offset = Offset(cos(degToRad(6 * i - 90)) * secondDistance + radius,
  ...
  void paint(Canvas canvas, Size size) {
    ...
    if (secondsOffset.length > 0) {
      ...
      for (var i = 0; i < secondsOffset.length; i++) {
        if (i % 5 == 0) {
          ...
          textPainter.layout();
          textPainter.paint(
              canvas, new Offset(-(textPainter.width / 2), -(textPainter.height / 2)));
          canvas.restore();
        }
        canvas.rotate(angle);
      }
    ...
    // print("r: $radius, h1: $hourHand1 ,h2: $hourHand2");
    ...
    Offset secondHand1 = Offset(
      // 秒针在圆心的坐标
      radius - cos(degToRad(360 / 60 * second - 90)) * (radius * 0.1),
      radius - sin(degToRad(360 / 60 * second - 90)) * (radius * 0.1),
    );
    Offset secondHand2 = Offset(
      //秒针距圆边坐标
      radius + cos(degToRad(360 / 60 * second - 90)) * (radius - borderWidth * 2),
      radius + sin(degToRad(360 / 60 * second - 90)) * (radius - borderWidth * 2),
    );
    //  print("r: $radius , s1: $secondHand1 ,s2: $secondHand2");
